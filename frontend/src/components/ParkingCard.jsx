...
return (
    <div className="group relative bg-gradient-to-br from-slate-50 to-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden border-2 border-slate-200/60 hover:border-blue-300">
      ...
      <div className="p-7">
        ...
        <div className="flex justify-between items-start mb-7">
          <div>
            ...
            <div className="flex items-center mt-2 text-slate-600">
              <MapPin className="w-4 h-4 mr-2 flex-shrink-0 opacity-70" />
              <p className="text-sm font-semibold truncate max-w-[180px]">{zone.location}</p>
            </div>
          </div>
          
          {/* Status Badge with Pulse */}
          <div className={`flex items-center gap-2 px-4 py-2 rounded-2xl text-xs font-black uppercase tracking-widest shadow-sm ${config.bg} ${config.color}`}>
        ...
        <div className="flex items-end justify-between mb-7">
          ...
          {/* Simple Circular Action Button (Visual only) */}
          <button className={`p-4 rounded-2xl bg-white shadow-md transition-all duration-300 group-hover:scale-110 group-hover:rotate-12 border-2 border-slate-100 ${config.button}`}>
        ...
        <div className="space-y-3">
          ...
          <div className="w-full bg-slate-200 rounded-full h-3 overflow-hidden shadow-inner">
            <div
              className={`h-full rounded-full ${config.accent} transition-all duration-700 ease-out group-hover:opacity-100 shadow-lg`}
              style={{ width: `${percentage}%` }}
            ></div>
          </div>
        </div>

        {/* Warning Message (Only shows when Full) */}
        {isFull && (
          <div className="mt-5 flex items-start gap-3 text-sm text-rose-700 bg-gradient-to-r from-rose-50 to-rose-100 p-4 rounded-2xl font-bold animate-in fade-in slide-in-from-top-2 border border-rose-200">
